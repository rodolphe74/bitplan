  ...
  for (uint16_t i = 0; i < newHeight; i++) {
    for (uint16_t j = 0; j < newWidth; j++) {
      ...
      r = ar * (1 - x_weight) * (1 - y_weight) +
          br * x_weight * (1 - y_weight) + cr * y_weight * (1 - x_weight) +
          dr * x_weight * y_weight;

      g = ag * (1 - x_weight) * (1 - y_weight) +
          bg * x_weight * (1 - y_weight) + cg * y_weight * (1 - x_weight) +
          dg * x_weight * y_weight;

      b = ab * (1 - x_weight) * (1 - y_weight) +
          bb * x_weight * (1 - y_weight) + cb * y_weight * (1 - x_weight) +
          db * x_weight * y_weight;
